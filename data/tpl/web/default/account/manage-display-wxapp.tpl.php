<?php defined('IN_IA') or exit('Access Denied'); ?><?php (!empty($this) && $this instanceof WeModuleSite || 0) ? (include $this->template('common/header', TEMPLATE_INCLUDEPATH)) : (include template('common/header', TEMPLATE_INCLUDEPATH)); ?>
    ...
    <ul class="we7-page-tab">
        <li class="active">
            <a href="<?php echo url('account/manage', array('account_type' => ACCOUNT_TYPE_APP_NORMAL)) ?>">小程序列表</a>
        </li>

        <?php if ($_W['role'] == ACCOUNT_MANAGE_NAME_OWNER || $_W['role'] == ACCOUNT_MANAGE_NAME_FOUNDER || $_W['role'] == ACCOUNT_MANAGE_NAME_VICE_FOUNDER) { ?>
            <li>
                <a href="<?php echo url('account/recycle', array('account_type' => ACCOUNT_TYPE_APP_NORMAL)) ?>">小程序回收站</a>
            </li>
        <?php } ?>
    ...
    <div class="clearfix we7-margin-bottom">
        ...
        <?php if (!$_W['isfounder'] && !empty($account_info['wxapp_limit']) || user_is_vice_founder()) { ?>
            <div class="alert alert-warning hidden">
                ...
                可创建 <span class="text-strong"><?php echo $account_info['maxwxapp']; ?> </span>个小程序，已创建<span
                        class="text-strong"> <?php echo $account_info['wxapp_num']; ?> </span>个，还可创建 <span
                        class="text-strong"><?php echo $account_info['wxapp_limit']; ?> </span>个小程序。
            </div>
        <?php } ?>
        ...
        <tr class="color-gray" ng-repeat="list in lists">
            ...
            <td class="vertical-middle vertical-middle table-manage-td">
                <div class="link-group">
                    ...
                    <?php if ($role_type) { ?>
                        <a ng-href="{{links.post}}&acid={{list.acid}}&uniacid={{list.uniacid}}"
                ...
                <?php if ($role_type) { ?>
                    <div class="manage-option text-right">
                        <a href="{{links.post}}&acid={{list.acid}}&uniacid={{list.uniacid}}"
