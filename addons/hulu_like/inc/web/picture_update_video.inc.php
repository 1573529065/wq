...
if (!empty($_GPC['picture_id'])) {
    ...
    if (!empty($picture)) {
        if ($picture['uniacid'] == $_W['uniacid']) {
            ...
            message('抱歉！您没有权限修改此信息！', $this->createWebUrl('picture_video', array('openid' => $_GPC['openid'])), 'error');
        }

    } else {
        message('抱歉！您要修改的信息不存在！', $this->createWebUrl('picture_video', array('openid' => $_GPC['openid'])), 'error');
    }

} else {
    message('警告！请勿提交非法网址！', $this->createWebUrl('picture_video', array('openid' => $_GPC['openid'])), 'error');
}
