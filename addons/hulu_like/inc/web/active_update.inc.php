...
if (!empty($_GPC['active_id'])) {
    ...
    if (!empty($active)) {
        if ($active['uniacid'] == $_W['uniacid']) {
            ...
            message('抱歉！您没有权限修改此信息！', $this->createWebUrl('active'), 'error');
        }
    } else {
        message('抱歉！您要修改的信息不存在！', $this->createWebUrl('active'), 'error');
    }
} else {
    message('警告！请勿提交非法网址！', $this->createWebUrl('active'), 'error');
}
