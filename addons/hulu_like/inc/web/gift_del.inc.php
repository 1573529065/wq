...
if (!empty($_GPC['gift_id'])) {
    ...
    if (!empty($gift)) {
        ...
        $res = pdo_delete('hulu_like_gift', array('uniacid' => $_W['uniacid'], 'gift_id' => $_GPC['gift_id']));
        if (!empty($res)) {
        ...
        message('抱歉！您要修改的信息不存在！', $this->createWebUrl('gift'), 'error');
    }

} else {
    message('警告！请勿提交非法网址！', $this->createWebUrl('gift'), 'error');
}
